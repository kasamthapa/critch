...
function ReviewsReceived({ projects }: { projects: ProjectWithReviews[] }) {
  ...
  const ScoreCard = ({
    ...
    <div className="border border-zinc-800 bg-zinc-900/60 px-3 sm:px-4 py-3">
      ...
      <div className="flex items-baseline gap-1">
        ...
        <span className="font-mono text-xs sm:text-sm text-zinc-600">/5</span>
      </div>
  ...
  const ReviewCard = ({ r }: { r: ProjectWithReviews["reviews"][number] }) => (
    <div className="border border-zinc-800 bg-zinc-900/30 p-4 sm:p-5">
      ...
      {r.comment && (
        <div className="mt-4 sm:mt-5 border-l border-zinc-700 pl-4 sm:pl-5">
          <p className="font-mono text-xs text-zinc-600 uppercase tracking-wider mb-2">
            feedback
          </p>
      ...
      <div className="flex flex-col gap-5 sm:gap-6">
        {projects.map((p) => (
          <div key={p.id} className="border border-zinc-800 bg-zinc-900/20">
            {/* Project header */}
            <div className="flex items-start justify-between gap-3 sm:gap-4 px-4 sm:px-6 py-4 sm:py-5 border-b border-zinc-800">
              ...
              <button
                ...
                className="shrink-0 inline-flex items-center gap-2 border border-zinc-700 bg-zinc-900 px-3 sm:px-4 py-2.5 font-mono text-xs sm:text-sm text-zinc-400 hover:border-zinc-500 hover:text-zinc-200 hover:bg-zinc-800 transition-all min-h-[44px]"
              >
                {expandedProject[p.id] ? "collapse" : "view all"}
                <span
                  className={`text-xs text-zinc-600 transition-transform duration-150 ${expandedProject[p.id] ? "rotate-180" : ""}`}
                >
                  ▾
                </span>
              </button>
